...
if st.session_state.get('data_loaded') and (parsed or aoi):
    ...
    def _panel_image_data_url(rendered_odb, layer_obj):
        """Return a data URL for panel background for chosen layer.

        Prefer panel SVG (step-repeat) then fallback to panel PNG.
        """
        use_vector = st.session_state.get('use_vector_panel', True)

        if use_vector and getattr(rendered_odb, 'panel_layout', None) and getattr(layer_obj, 'svg_string', None):
            try:
                from gerber_renderer import build_panel_svg
                svg_data_url = build_panel_svg(layer_obj.svg_string, rendered_odb.panel_layout)
                if svg_data_url and svg_data_url.startswith('data:image/svg+xml'):
                    return svg_data_url
            except Exception as e:
                st.write(f"[debug] build_panel_svg failed: {e}")

        # Fallback: pre-rendered panel PNG from gerber_renderer or prior build
        png_url = getattr(layer_obj, 'panel_png_data_url', None)
        if png_url:
            return png_url
    ...
    if view_mode == "🔭 Panel Overview":
        ...
        if aoi and aoi.has_data:
            ...
            # Toggle: prefer vector tiled SVG (built from step-repeat) when available
            st.checkbox("Prefer vector tiled SVG (use step-repeat when available)",
                        key='use_vector_panel', value=True,
                        help='If enabled the app will try to render a vector tiled SVG for the panel background; falls back to PNG.')
            ...
            if _rendered_panel and _rendered_panel.panel_layout:
                ...
                # If user hasn't checked any layer, default to the first copper layer
                # so the panel shows copper nets by default.
                if not _all_checked_panel:
                    # Look for a copper layer name
                    for _ln_name, _ln_obj in _rendered_panel.layers.items():
                        if getattr(_ln_obj, 'layer_type', '') == 'copper':
                            _all_checked_panel = [_ln_name]
                            break
                if _all_checked_panel:
                    _want_layer = _all_checked_panel[0]   # PNG/SVG can only show one layer
                ...
                if _want_layer:
                    ...
                    # Attempt to prefer a vector tiled SVG for the panel background.
                    # _panel_image_data_url will try SVG (if enabled) and fall back to PNG.
                    _panel_png_url = _panel_image_data_url(_rendered_panel, _want_lyr_obj)
                    st.write('DEBUG: selected layer', _want_layer, 'type', _want_lyr_obj.layer_type)
                    st.write('DEBUG: panel_layout exist', bool(_rendered_panel.panel_layout))
                    st.write('DEBUG: use_vector_panel', st.session_state.get('use_vector_panel', True))
                    st.write('DEBUG: panel_url start', _panel_png_url[:100] if _panel_png_url else None)

                    # If the helper didn't return anything (no SVG and no existing PNG),
                    # build a PNG on demand as before and re-query.
                    if not _panel_png_url:
                        if not getattr(_want_lyr_obj, 'panel_png_data_url', None):
            ...
            if _pl_cam and _rodb.layers:
                ...
                # Toggle: prefer vector tiled SVG when available (shared key with panel overview)
                st.checkbox("Prefer vector tiled SVG (use step-repeat when available)",
                            key='use_vector_panel', value=True,
                            help='If enabled the app will try to render a vector tiled SVG for the panel background; falls back to PNG.')
                ...
                if _sel_ln2:
                    ...
                    # Try to obtain a vector SVG data URL first (helper will fall back to existing PNG)
                    _panel_png = _panel_image_data_url(_rodb, _sel_lyr2)

                    # If no SVG and no PNG yet, build the PNG on demand (existing behaviour)
                    if not _panel_png:
                        if not getattr(_sel_lyr2, 'panel_png_data_url', None):
            ...
            if _rodb_cm_check and _rodb_cm_check.layers:
                ...
                if not _na_checked:
                    ...
                    _design_fig.update_layout(
                        ...
                        yaxis=dict(range=[-1, _no_aoi_ch + 1], showgrid=False, zeroline=False, showticklabels=False),
