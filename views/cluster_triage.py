...
def render_cluster_triage(parsed, aoi, align_args) -> None:
    ...
    if not _has_aoi_ct:
        ...
        if (_rodb_ct and _rodb_ct.panel_layout and _rodb_ct.layers
            ...
            # Pin index normalisation to the UNFILTERED dataset so scoping a panel/BU
            # never shifts the origin mapping (e.g. when scope drops all row-0 defects).
            _ct_min_iy = int(aoi.all_defects['UNIT_INDEX_Y'].min())
            _ct_min_ix = int(aoi.all_defects['UNIT_INDEX_X'].min())
            _ct_pairs = list(zip(
            ...
            _ct_df['ALIGNED_X'] = _ct_df['X_MM'] if 'X_MM' in _ct_df.columns else pd.NA
            _ct_df['ALIGNED_Y'] = _ct_df['Y_MM'] if 'Y_MM' in _ct_df.columns else pd.NA
        ...
        if len(_ct_xy) < 5:
            ...
            for _cid in sorted(set(_labels)):
                if _cid == -1:
                    continue
                _cl = _ct_df[_ct_df['CLUSTER_ID'] == _cid]
                _cnt = len(_cl)
