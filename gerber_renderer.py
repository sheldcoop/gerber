...
def render_odb_to_cam(data: bytes, filename: str = '',
    ...
    try:
        ...
        if step_hierarchy:
            ...
            # Post-profile inch check: catches ANGLE=270 designs where all DX=DY=0
            # (InCAM Pro writes NX=1/NY=1 per unit so DX/DY are always 0, defeating
            # the spacing heuristic above). If the max absolute SR coordinate is
            # smaller than unit_w, the hierarchy is still in raw inch scale.
            if unit_w > 10:
                _sr_max_abs = 0.0
                for _sr_list in step_hierarchy.values():
                    for _sr in _sr_list:
                ...
                if 0 < _sr_max_abs < unit_w:
                    step_hierarchy = _parse_step_repeat(job_root, 25.4)
                    warnings.append(
                        f"⚠️ Step-repeat inch quirk (post-profile): max SR coord "
                        f"{_sr_max_abs:.3f} < unit_w {unit_w:.2f} mm — re-parsed with uf=25.4"
                    )
